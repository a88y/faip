...
$id_detail			= $_POST['id_detail'];
	...
	$query = "update 2_2_pqr set akibat_penilai = ".(($txtSelectNilaiP=='')?"NULL":("'".$txtSelectNilaiP."'")).",
					...
					where id_2_2_detail = '".$id_detail."' limit 1;";
	...
	$query_1 = "select id_2_2 as id from 2_2_detail where id_2_2_detail = '".$id_detail."' limit 1;";
	$result_1 = mysqli_query($dbLink, $query_1);
	$row_1 = mysqli_fetch_array($result_1);
	
	$id = $row_1['id'];
	
	$query_6 = "delete from 2_2_kompetensi where id_2_2_detail = '".$id_detail."'";
		...
		for($i=0;$i<count($kopetensiStr);$i++){ 
			...
			if(mysqli_num_rows($result_Kop) > 0 ) {
				while($row_Kop = mysqli_fetch_array($result_Kop)) {
					$maxSQL = "SELECT max(id_kompetensi) as maxID FROM 2_2_kompetensi WHERE id_kompetensi LIKE '".$id_detail."%'";
					...
					$query_6 = "insert into 2_2_kompetensi (id_kompetensi, id_2_2_detail, kompetensi, nilai_kompetensi) values ( '".$newIDKop."', '".$id_detail."', '".$row_Kop["kode"]." ".$row_Kop["kompetensi"]."' , '".$row_Kop["satuan"]."' );";
	...
	$query_2 = "select id_2_2_detail as id_detail from 2_2_detail where id_2_2 = '".$id."';";
	...
	while ( $row_2 = mysqli_fetch_array($result_2) ) {
		...
		$query_3 = "select IFNULL(nilai_p_penilai,nilai_p) nilai_p, IFNULL(nilai_q_penilai, nilai_q) nilai_q, IFNULL(nilai_r_penilai, nilai_r) nilai_r from 2_2_pqr where id_2_2_detail = '".$row_2['id_detail']."';";
		...
		$query_4 = "select SUM( nilai_kompetensi ) as kompetensi_terhitung from 2_2_kompetensi where id_2_2_detail = '".$row_2['id_detail']."';";
	...
	$query_5 = "update 2_2 set total_penilaian_penilai = '".$calculate."' where id_2_2 = '".$id."' limit 1;";
