...
$id_detail			= $_POST['id_detail'];
	...
	$query = "update 1_4_pqr set jenis_penilai = ".(($txtSelectNilaiP=='')?"NULL":("'".$txtSelectNilaiP."'")).",
					...
					where id_1_4_detail = '".$id_detail."' limit 1;";
					
	//echo $query;

	mysqli_query($dbLink,$query);
	
	$query_1 = "select id_1_4 as id from 1_4_detail where id_1_4_detail = '".$id_detail."' limit 1;";
	$result_1 = mysqli_query($dbLink, $query_1);
	$row_1 = mysqli_fetch_array($result_1);
	
	$id = $row_1['id'];
	
	$query_6 = "delete from 1_4_kompetensi where id_1_4_detail = '".$id_detail."'";
		...
		for($i=0;$i<count($kopetensiStr);$i++){ 
			...
			if(mysqli_num_rows($result_Kop) > 0 ) {
				while($row_Kop = mysqli_fetch_array($result_Kop)) {
					$maxSQL = "SELECT max(id_kompetensi) as maxID FROM 1_4_kompetensi WHERE id_kompetensi LIKE '".$id_detail."%'";
					...
					$query_6 = "insert into 1_4_kompetensi (id_kompetensi, id_1_4_detail, kompetensi, nilai_kompetensi) values ( '".$newIDKop."', '".$id_detail."', '".$row_Kop["kode"]." ".$row_Kop["kompetensi"]."' , '".$row_Kop["satuan"]."' );";
	...
	$query_2 = "select id_1_4_detail as id_detail from 1_4_detail where id_1_4 = '".$id."';";
	...
	while ( $row_2 = mysqli_fetch_array($result_2) ) {
		...
		$query_3 = "select IFNULL(nilai_p_penilai,nilai_p) nilai_p, IFNULL(nilai_q_penilai, nilai_q) nilai_q, IFNULL(nilai_r_penilai, nilai_r) nilai_r from 1_4_pqr where id_1_4_detail = '".$row_2['id_detail']."';";
		...
		$query_4 = "select SUM( nilai_kompetensi ) as kompetensi_terhitung from 1_4_kompetensi where id_1_4_detail = '".$row_2['id_detail']."';";
	...
	$query_5 = "update 1_4 set total_penilaian_penilai = '".$calculate."' where id_1_4 = '".$id."' limit 1;";
