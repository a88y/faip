...
<?php
	$query_1 = "select t1.id_user, t1.id_3_4, IFNULL(t1.total_penilaian_penilai, t1.total_penilaian) total_penilaian, t2.id_3_4_detail, t2.kegiatan, t2.nama, t2.perguruan, t2.sejak, t2.sampai, t2.client, t2.pemilik, t3.id_jenis_parent, t3.jenis, t3.nilai_p, t3.peran, t3.nilai_q, t3.cakupan, t3.nilai_r, t3.id_jenis_parent_penilai, t3.jenis_penilai, t3.nilai_p_penilai, t3.peran_penilai, t3.nilai_q_penilai, t3.cakupan_penilai, t3.nilai_r_penilai, t4.parent
				...
				where t1.id_user = '".$id_user."' order by t2.id_3_4_detail asc";
					...
					<div class="span12">
						...
						<?php  while ( $row_1 = mysqli_fetch_array($result_1) ) { 
									...
									if($row_1["id_jenis_parent_penilai"] != ''){
										$query_uu = "select * from 3_4_p_parent where id_3_4_p_parent = '".$row_1["id_jenis_parent_penilai"]."' limit 1";
...
?>
                                                <li class="<?php if($row_p_parent['id_3_4_p_parent'] == $row_1["id_jenis_parent_penilai"]) { echo "selected"; } ?>"><div data-value="<?php echo $row_p_parent['parent']?>" data-nilai="<?php echo $row_p_parent['id_3_4_p_parent']?>"><?php echo $row_p_parent['parent']?></div></li>
<?php
								}
?>
                                            	</ul>
...
?>
													<li class="<?php if($row_p['jenis'] == $row_1["jenis_penilai"]) { echo "selected"; } ?>" data-filter="<?php echo $row_p_parent_s["id_3_4_p_parent"]?>" style=" <?php if($row_p["id_3_4_p_parent"] == $row_1["id_jenis_parent_penilai"] ){ echo 'display:block;'; } else { echo 'display:none;'; } ?> "><div data-value="<?php echo $row_p['jenis']?>" data-nilai="<?php echo $row_p['nilai_p']?>"><?php echo $row_p['jenis']?></div></li>                                           	
<?php
									}
...
<?php
	$i = 0;
	$query_2 = "select id_kompetensi, kompetensi, nilai_kompetensi from 3_4_kompetensi where id_3_4_detail = '".$row_1["id_3_4_detail"]."' order by id_kompetensi asc";	
...
while($row_k = mysqli_fetch_array($result_k)) {
	...
	$query_uu = "select * from 3_4_kompetensi where id_3_4_detail = '".$row_1["id_3_4_detail"]."' && kompetensi = '".$valueKopetensi."' limit 1";
	$result_uu = mysqli_query($dbLink,$query_uu);
	if( mysqli_num_rows($result_uu) > 0 ) {
...
?>
                                    ...
                                    <div class="control-group" style="background-color:transparent;">
                                        <div class="controls" style="border-left:none;">
                                        	<input type="hidden" name="id_detail" value="<?=$row_1["id_3_4_detail"]?>" />
